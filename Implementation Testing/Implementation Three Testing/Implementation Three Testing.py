...
def Clique_Overlap_test_One(G,clique_list):
    
    print('--------------------------------------------------------------\n')
    print('CHECKING Clique OVERLAP CONDITION (TEST 1)')
    print('--------------------------------------------------------------')

    for i in range(len(clique_list)):
        clique_1 = clique_list[i]
        clique_1_size = len(clique_1)

        for j in range(len(clique_list)):

            if i != j:
                clique_2 = clique_list[j]
                clique_2_size = len(clique_2)
                clique_overlap = 0
                for k in range(clique_1_size):
                    for l in range(clique_2_size):
                        if G.has_edge(k,l):
                            clique_overlap+=1

                if (clique_1_size > clique_2_size):
                    if (clique_overlap >= 0.2 * clique_1_size):
                        pass
                    else:
                        print('\nPossibility of fork: ' + 'Insufficient overlap between cliques' + str(
                            clique_1) + ' and ' + str(clique_2))
                        return

                else:
                    if (clique_overlap >= 0.2 * clique_2_size):
                        pass
                    else:
                        print('\nPossibility of fork: ' + 'Insufficient overlap between cliques ' + str(
                            clique_1) + ' and ' + str(clique_2))
...
def clique_topology(network_size, clique_seq, overlap_seq, col_list):
    ...
    UNL_Overlap_test_One(G,clique_list)
...
if __name__ == "__main__":
    ...
    param_list = [num_nodes_list, mal_node_list, network_topology_list, clique_list_range, e2c_latecy_list, c2c_latecy_list]
